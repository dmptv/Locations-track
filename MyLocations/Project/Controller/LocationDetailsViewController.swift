...
class LocationDetailsViewController: UITableViewController {
    ...
    fileprivate func string(from placemark: CLPlacemark) -> String {
        ...
        // Newlines aren’t necessary here because the label will word-wrap
        return line
    }
    ...
    @IBAction func done() {
        ...
        if let image = image {
            ...
            // If a location's photo existed, you simply keep the same ID and overwrite the existing JPEG file
            if let data = UIImageJPEGRepresentation(image, 0.5) {
                do {
                    // save Data object to path given by photoURL property
